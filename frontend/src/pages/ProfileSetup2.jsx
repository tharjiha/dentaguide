...
import { saveStep2 } from '../api/profile';
import { StepSidebar } from './ProfileSetup1';
import { GumIcon, DecayIcon, WarnIcon, GeneticIcon, DryMouthIcon, GrindIcon, BoltIcon, PostTreatmentIcon, WellnessIcon, CheckIcon } from '../components/Icons';

const DEFAULT_CONDITIONS = [
  { key: 'gum',         label: 'Gum disease',     icon: <GumIcon size={17} />,           sel: false },
  { key: 'decay',       label: 'Tooth decay',      icon: <DecayIcon size={17} />,         sel: false },
  { key: 'enamel',      label: 'Enamel erosion',   icon: <WarnIcon size={17} />,          sel: false },
  ...
  { key: 'sensitivity', label: 'Sensitivity',      icon: <BoltIcon size={17} />,          sel: false },
...
export default function ProfileSetup2() {
  ...
  const addCustom = () => {
    ...
    if (!val) return;
    setConditions(prev => [...prev, { key: `custom_${Date.now()}`, label: val, icon: <CheckIcon size={17} />, sel: true }]);
    setCustomInput('');
  };

  const handleContinue = async () => {
    const selected = conditions.filter(c => c.sel).map(c => c.label);
    if (!selected.length) { setError('Please select at least one condition'); return; }
    setError(''); setLoading(true);
    try {
      await saveStep2({ conditions: selected }, accessToken);
      navigate('/profile/3');
  ...
  return (
    <div className="sidebar-layout">
      ...
      <div>
        ...
        <div className="sec-sub">Select all that apply. Your AI agents are tuned to what you pick here.</div>
        ...
        <div className="card">
          <div className="cond-grid" id="cond-grid-2">
            {conditions.map(c => (
          ...
          <div style={{ marginTop: '16px' }}>
            ...
            <div style={{ display: 'flex', gap: '8px' }}>
              <input className="form-input" style={{ flex: 1 }} placeholder="e.g. Orthodontic treatment, implants…"
